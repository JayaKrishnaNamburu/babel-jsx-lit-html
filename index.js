...
module.exports = api => {
  ...
  function renderProp(prop) {
    ...
    if (prop.value) {
      ...
      if (prop.value.type == "StringLiteral") {
        ...
        if (attributeName)
          return [" ", `${attributeName}`, "=", prop.value.value];
        ...
        if (propertyName)
          return [
            ...
            `.${propertyName}`,
            "=",
            t.stringLiteral(prop.value.extra.rawValue)
          ];
      ...
      if (prop.value.type == "JSXExpressionContainer") {
        ...
        if (attributeName)
          return [" ", `${attributeName}`, "=", prop.value.expression];
        if (propertyName)
          return [" ", `.${propertyName}`, "=", prop.value.expression];
        if (eventName)
          return [" ", `@${eventName}`, "=", prop.value.expression];
      }
    } else {
      // prop has no value
  ...
  function renderChild(child) {
    if (child.type == "JSXText") return [child.value]; // text becomes part of template
